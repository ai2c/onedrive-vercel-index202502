...
const GridItem = ({ c, path }: { c: OdFolderChildren; path: string }) => {
  ...
  return (
    <Dialog.Panel className="space-y-2">
      <Dialog.Panel className="h-32 overflow-hidden rounded border border-gray-900/10 dark:border-gray-500/30">
        {thumbnailUrl && !brokenThumbnail ? (
          ...
          <Dialog.Panel className="relative flex h-full w-full items-center justify-center rounded-lg">
            <ChildIcon child={c} />
      ...
      <Dialog.Panel className="flex items-start justify-center space-x-2">
        <span className="w-5 flex-shrink-0 text-center">
          <ChildIcon child={c} />
        </span>
        <ChildName name={c.name} folder={Boolean(c.folder)} />
      </Dialog.Panel>
      <Dialog.Panel className="truncate text-center font-mono text-xs text-gray-700 dark:text-gray-500">
...
const FolderGridLayout = ({
  ...
  return (
    <Dialog.Panel className="rounded bg-white shadow-sm dark:bg-gray-900 dark:text-gray-100">
      <Dialog.Panel className="flex items-center border-b border-gray-900/10 px-3 text-xs font-bold uppercase tracking-widest text-gray-600 dark:border-gray-500/30 dark:text-gray-400">
        <Dialog.Panel className="flex-1">{t('{{count}} item(s)', { count: folderChildren.length })}</Dialog.Panel>
        <Dialog.Panel className="flex p-1.5 text-gray-700 dark:text-gray-400">
          <Checkbox
      ...
      <Dialog.Panel className="grid grid-cols-2 gap-3 p-3 md:grid-cols-4">
        {folderChildren.map((c: OdFolderChildren) => (
          ...
          >
            <Dialog.Panel className="absolute top-0 right-0 z-10 m-1 rounded bg-white/50 py-0.5 opacity-0 transition-all duration-100 group-hover:opacity-100 dark:bg-gray-900/50">
              {c.folder ? (
                <Dialog.Panel>
                  <span
